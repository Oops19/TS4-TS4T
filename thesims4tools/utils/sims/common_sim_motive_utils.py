...
def _common_set_motive_level(output: CommonConsoleCommandOutput, motive: TunableInstanceParam(Types.STATISTIC), level: float, sim_info: SimInfo = None):
    if motive is None:
        output('ERROR: Failed, Motive not specified or Motive did not exist! ts4t.set_motive_level <motive_name_or_id> <level> [opt_sim=None]')
        return
    if sim_info is None:
        return
