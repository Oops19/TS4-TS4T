...
from thesims4tools.enums.tags_enum import CommonGameTag
...
class CommonOutfitUtils(HasClassLog):
    ...
    def copy_outfit(cls, mod_identity: CommonModIdentity, sim_info: SimInfo, from_outfit_category_and_index: Tuple[OutfitCategory, int], to_outfit_category_and_index: Tuple[OutfitCategory, int], change_sim_to_outfit_after_apply: bool = False) -> bool:
        ...
        :rtype: bool
        """
        from thesims4tools.services.sim.cas.common_sim_outfit_io import CommonSimOutfitIO
    ...
    def regenerate_outfit(cls, sim_info: Union[SimInfo, SimInfoBaseWrapper], outfit_category_and_index: Tuple[OutfitCategory, int]) -> None:
        ...
        tags = tuple()
        from thesims4tools.utils.sims.common_sim_gender_option_utils import CommonSimGenderOptionUtils
...
outfit_commands_log = CommonLogRegistry().register_log(ModInfo.get_identity(), 'ts4t_outfit_commands')
outfit_commands_log.enable()


@CommonConsoleCommand(
    ModInfo.get_identity(),
    'ts4t_testing.print_all_outfit_categories',
    'Print a list of all outfit categories that are available to Sims.'
)
def _ts4t_testing_print_all_outfit_categories(output: CommonConsoleCommandOutput):
...
@CommonConsoleCommand(
    ModInfo.get_identity(),
    'ts4t_testing.print_outfit_tags',
    ...
    command_aliases=(
        'ts4t_testing.printoutfittags',
    )
)
def _ts4t_testing_print_outfit_tags(output: CommonConsoleCommandOutput, outfit_category: str = None, outfit_index: int = 1, sim_info: SimInfo = None):
    if sim_info is None:
        return
...
@CommonConsoleCommand(
    ModInfo.get_identity(),
    'ts4t_testing.print_outfit_tags_by_cas_part',
    ...
    command_aliases=(
        'ts4t_testing.printoutfittagsbycaspart',
    )
)
def _ts4t_testing_print_outfit_tags_by_cas_part(output: CommonConsoleCommandOutput, outfit_category: str = None, outfit_index: int = 1, sim_info: SimInfo = None):
...
@CommonConsoleCommand(
    ModInfo.get_identity(),
    'ts4t.print_previous_outfit',
    'Print information about the previous outfit a Sim was wearing.',
    command_arguments=(
        CommonConsoleCommandArgument('sim_info', 'Sim Id or Name', 'The Sim to check.', is_optional=True, default_value='Active Sim'),
    ),
    command_aliases=(
        'ts4t.printpreviousoutfit',
    )
)
def _ts4t_print_previous_outfit(output: CommonConsoleCommandOutput, sim_info: SimInfo = None):
    if sim_info is None:
...
def _ts4t_print_outfit(output: CommonConsoleCommandOutput, outfit_category: str = None, outfit_index: int = 1, sim_info: SimInfo = None):
    if sim_info is None:
...
def _ts4t_print_outfits(output: CommonConsoleCommandOutput, show_missing_outfit_info: bool = False, sim_info: SimInfo = None):
    if sim_info is None:
...
def _ts4t_generate_outfit(output: CommonConsoleCommandOutput, outfit_category: str = None, outfit_index_val: int = 1, sim_info: SimInfo = None):
...
@CommonConsoleCommand(
    ModInfo.get_identity(),
    'ts4t.regenerate_all_outfits',
    'Regenerate all outfits of a Sim.',
    command_arguments=(
        CommonConsoleCommandArgument('sim_info', 'Sim Id or Name', 'The Sim to regenerate the outfit of.', is_optional=True, default_value='Active Sim'),
    ),
    command_aliases=(
        'ts4t.regeneratealloutfits',
    )
)
