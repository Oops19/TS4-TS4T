...
from thesims4tools.exceptions.common_stacktrace_utils import CommonStacktraceUtil
...
from thesims4tools.utils.common_io_utils import CommonIOUtils
...
class CommonLog:
    ...
    def __init__(self, mod_identifier: Union[str, CommonModIdentity], log_name: str, custom_file_path: str = None):
        ...
        from thesims4tools.utils.misc.common_mod_identity_utils import CommonModIdentityUtils
...
class CommonLogRegistry(CommonService):
    ...
    .. code-block:: python
        ...
        # Register the log, Logs will appear in a file titled "MOD_NAME_Messages.txt" and messages logged using this log will be prefixed with "ts4t_log_name"
        log = CommonLogRegistry.get().register_log('MOD_NAME', 'ts4t_log_name')
    ...
    .. note::
        ...
        - `ts4t.enable_log` or `ts4t.enablelog`
        - `ts4t.disable_log` or `ts4t.disablelog`
        - `ts4t.disable_all_logs` or `ts4t.disablealllogs`
        - `ts4t.logs`
...
_log = CommonLogRegistry().register_log(ModInfo.get_identity(), 'ts4t_log_registry')
