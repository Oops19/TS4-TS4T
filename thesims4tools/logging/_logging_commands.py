...
def _common_command_enable_log(output: CommonConsoleCommandOutput, log_name: str, mod_name: str=None):
    if log_name is None:
        output('ERROR: No log name specified (See all logs via "ts4t.logs" command)')
        return
    if mod_name is None:
...
def _common_command_disable_log(output: CommonConsoleCommandOutput, log_name: str, mod_name: str=None):
    if log_name is None:
        output('specify a log name (See all logs via "ts4t.logs" command)')
        return
    if mod_name is None:
